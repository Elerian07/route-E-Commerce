...
export const addBrand = asyncHandler(async (req, res, nex) => {
    if (!req.file) {
        ...
        return res.status(201).json({ message: "Brand Created", result })
    }
})
//update
export const updateBrand = asyncHandler(async (req, res, next) => {
    ...
    if (!brand) {
        ...
        return res.status(200).json({ message: "updated", updatedBrand })
    }
})
//delete
export const deleteBrand = asyncHandler(async (req, res, next) => {
    ...
    if (!brand) {
        ...
        if (brand.addBy.equals(req.user._id)) {
            ...
            return res.status(200).json({ message: "Brand has been deleted", deletedBrand })
        } else {
            return next(new Error("you are not authorized to delete this Brand", { cause: 403 }))
        }
